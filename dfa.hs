...
checkDFA :: DFA -> IO ()
checkDFA (states, alphabet, delta, start, accept) = do
...
statesOf delta = concatMap statesOfTransn delta
  where statesOfTransn ((st, _), st') = [st, st']

nonDets :: [Transn] -> [(State, Symbol)]
nonDets delta = domain \\ nub domain where domain = map fst delta

symbols :: [Transn] -> [Symbol]
symbols delta = nub (sort syms)
 where
...
checkValid :: [State] -> IO ()
checkValid states = do
  let illegalStates = filter (< 0) states
  if null illegalStates
    then return ()
    else do
      putStrLn "ERROR:"
...
checkDeclared states = if null states
  then return ()
  else do
    putStrLn "ERROR:"
...
checkNondets entries = if null entries
  then return ()
  else do
    putStrLn "ERROR:"
