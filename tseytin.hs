...
prepare (NOT (NOT e   )) = prepare e

-- push negations over AND, OR (De Morgan) and BIIM, XOR
prepare (NOT (OR   e f)) = AND (prepare (NOT e)) (prepare (NOT f))
-- COPY OVER YOUR PREVIOUS SOLUTION
prepare (NOT (AND  e f)) = OR (prepare (NOT e)) (prepare (NOT f))
prepare (NOT (BIIM e f)) = XOR (prepare e) (prepare f)
prepare (NOT (XOR  e f)) = BIIM (prepare e) (prepare f)

-- otherwise, pass over these connectives
prepare (AND  e f      ) = AND (prepare e) (prepare f)
prepare (OR   e f      ) = OR (prepare e) (prepare f)
prepare (BIIM e f      ) = BIIM (prepare e) (prepare f)
prepare (XOR  e f      ) = XOR (prepare e) (prepare f)
...
prepare (NOT (IMPL e f)) = AND (prepare e) (prepare (NOT f))
prepare (IMPL e f      ) = OR (prepare (NOT e)) (prepare f)

-- stop once the negations get down to the variables
-- (variables and negated variables are OK)
prepare (VAR x         ) = VAR x
prepare (NOT (VAR x)   ) = NOT (VAR x)
...
tseytin exp = [ttid tree] : transform tree
 where
  ...
  maketree (NOT (VAR x)) i = (TTVar (-(ord x)), i)
  maketree (VAR x      ) i = (TTVar (ord x), i)
  maketree (AND e f    ) i = (TTAnd k et ft, k + 1)
   where
    (et, j) = maketree e i
    (ft, k) = maketree f j
  maketree (OR e f) i = (TTOr k et ft, k + 1)
   where
    (et, j) = maketree e i
    (ft, k) = maketree f j
  maketree (XOR e f) i = (TTXor k et ft, k + 1)
   where
    (et, j) = maketree e i
    (ft, k) = maketree f j
  maketree (BIIM e f) i = (TTBiim k et ft, k + 1)
   where
    (et, j) = maketree e i
    (ft, k) = maketree f j
  ...
  transform (TTVar i) = [] -- variables are captured by the clauses of their parent nodes
