...
scan []                    = []
scan s = Bad stuff : scan rest where (stuff, rest) = break isSpace s
...
parse :: String -> Exp
parse input = if null remaining_tokens
  then exp
...
pExp (Ident c   : tokens) = (VAR c, tokens)

pExp (LeftParen : tokens) = (exp, consume_RightParen remaining_tokens)
 where
...
pExp tokens = error
  ("Identifier or left parenthesis expected, found " ++ show next)
  where next = head tokens
...
pPar (NegToken : tokens) = (NOT exp, remaining_tokens)
  where (exp, remaining_tokens) = pExp tokens

pPar tokens = (complete exp, remaining_tokens)
 where
...
pRem (AndToken : tokens) = (\e1 -> AND e1 e2, remaining_tokens)
  where (e2, remaining_tokens) = pExp tokens

pRem (OrToken : tokens) = (\e1 -> OR e1 e2, remaining_tokens)
  where (e2, remaining_tokens) = pExp tokens

pRem (XorToken : tokens) = (\e1 -> XOR e1 e2, remaining_tokens)
  where (e2, remaining_tokens) = pExp tokens

pRem (BiimToken : tokens) = (\e1 -> BIIM e1 e2, remaining_tokens)
  where (e2, remaining_tokens) = pExp tokens

pRem (ImplToken : tokens) = (\e1 -> IMPL e1 e2, remaining_tokens)
  where (e2, remaining_tokens) = pExp tokens

pRem (RightParen : tokens) = (id, tokens)

pRem (t : _) = error ("Unexpected token: " ++ show t)

pRem [] = error "Unexpected end of input, expected connective or RightParen"
